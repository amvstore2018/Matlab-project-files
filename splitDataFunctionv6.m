...
for k = 1 : n/2
    ...
    Tf_roll=roll(1)+dcgain_roll/(s^2+2*zeta_roll*wn_roll*s+wn_roll^2);
    Tf_p=dcgain_p*(s)*(Tf_roll-roll(1))/dcgain_roll;
    ...
    if sign(B(2,st3))==-1
        ...
        subplot(3,2,contr1)
        plot(Time1,roll,t_roll+20,y_roll)
        if state ==1
...
%        legend('Act','Equ','Location','Best','FontSize',6); % suptitle([' T_{s} =',num2str(Ts(1)),'sec , T_{p} =',num2str(Tp(1)),'sec']);
 ...
 %       legend('Act','Equ','Location','Best','FontSize',6); % suptitle([' T_{s} =',num2str(Ts(1)),'sec , T_{p} =',num2str(Tp(1)),'sec']);
        ...
        set(gcf,'units','normalized','outerposition',[0 0 1 1]);
        subplot(3,2,contr2)
        plot(Time1,roll,t_roll+10,y_roll)
        if state==1
            ylabel('roll (deg)','FontSize',18,'FontWeight','bold');
            title(' roll versus time','FontSize',20,'FontWeight','bold');
            xlabel('time (sec)','FontSize',18,'FontWeight','bold');
